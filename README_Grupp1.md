...
Systemet Kvartesbion biografbokningssystem byggt med React och TS i frontend, C# i backend, kopplat
till en MySQL databas.
Användaren kan navigera och kolla på olika filmer som visas, kolla trailer, läsa information om
filmen, boka bioplatser, skapa inloggning, logga in och vid inloggning se bokningshistorik samt
avboka bokade filmer.
...
Thomas admin konto finns fortfarande sedan projektstart.

Teknisk skuld:
Vi har försökt använda oss av dry men det har inte hållt sig överallt. Vi har på många platser i
css komponenterna använt oss av kod som gör samma sak för att bibehålla en viss struktur.
För framtiden kan vi strukturera upp kod bättre och kanske även bryta ut mer css i komponenter för
att ha de mer enhetligt.

Lösningarkitektur:
Systemet är uppbyggt enligt en klient-server-arkitektur där frontend och backend är separerade.
Frontend ansvarar för användargränssnittet medan backend hanterar affärslogik och datahantering.
Kommunikation sker via ett REST API över HTTP.
...
Backend:
Backend är utvecklad i C# med .NET
...
Databas:
En molnbaserad MySQL-databas

Den används för lagring av:
Filmer
Visningstider
Bokningar
Användardata
...
Vi valde att fokusera på grund kraven och att uppnå alla krav fullständigt innan vi går in på fler
punkter i projektet.
Detta gjorde att vi fick ett mer komplett projekt istället för att ha fler funktioner som inte är
hela.
...
1.  Starting of to get the repo to your local machine you need to clone it.
...
2.  then open your terminal and run this command:
...
1.  When you have entered the directory for the projekt type:
    npm install

You should now see a folder named node_modules and package-lock.json

2.  Next step is to change the content of the file named: db-configTemplate.json
    this file should match your database connection.

3.  After changing the content you need to change the name of that file so it's named:
    db-config.json

-================================ Ready to start ========================================-

1.  To run the frontend you need to open the terminal and run the command:
    npm run dev

2.  Wait until the terminal has finished loading and there appears some orange text, then you can open
    the local host in your web browser by pressing CTRL and left mouse click on localhost-link to open
    in browser.

3.  Now your ready to browse the website and make some bookings for the films listed.
...
If your dotnet version is lower than 10 you need to install it first and restart your pc.

---

You can also make sure that you have the correct react and vite version by typing:
...
the false to a true, or make it not commented in the file DbQuery.cs
